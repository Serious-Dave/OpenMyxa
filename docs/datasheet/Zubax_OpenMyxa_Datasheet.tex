...
\begin{itemize}
    \item 2800 W continuous power output.
    \item 4S - 12S wide power voltage range.
    \item 5v, 0.5 A BEC.
    ...
    \begin{itemize}
        ...
        \item Protection against unlicensed (counterfeit) production by means of a digital signature installed on every 
        manufactured unit.
     \end{itemize}
\end{itemize}
...
\end{itemize}
\centering
\includegraphics[width=0.42\textwidth]{openmyxa-top}
\includegraphics[width=0.42\textwidth]{openmyxa-bottom}
\end{titlepage}
...
\chapter{Overview}
Zubax OpenMyxa is an open hardware reference design for Mitochondrik. It is a tightly integrated unit that contains an electric motor with sensorless electronic speed controller in a single monolithic package. 

Mitochondrik \footnote{Refer to \url{https://zubax.com/products/mitochondrik} for more information} is an integrated module (like an integrated circuit) that enables third-party hardware engineers to design sophisticated custom motor controllers using motor control technology -Telega\footnote{Reefer to \url{https://zubax.com/technologies/telega} for more information}, which integrates advanced control algorithms.   
This datasheet is focused only on the OpenMyxa hardware.
...
There are two versions of OpenMyxa: enclosed and bare board. 
...
Zubax OpenMyxa can be used with the following accessories:
...
\begin{ZubaxSimpleTable}{Absolute maximum ratings}{|c X|c c|c|}
    ...
    $V_\text{inv}$    & Supply voltage           & -0.3 & 53  & V \\
...
\begin{ZubaxTableWrapper}{Electrical characteristics}
    \begin{ZubaxWrappedTable}{|c X|c c c|c|}
        ...
        $P$                 & Continuous power                    &      &      & 2800  & W \\
        $P_\text{peak}$     & Peak power (30sec)                  &      &      & 3200  & W \\
        $I_\text{inv}$      & Continuous DC current               &      &      & 70   & A \\
        $I_\text{inv-peak}$ & Peak DC current                     &      &      & 100   & A \\
        $I_\text{idle}$     & Idle current consumption            &      & 50   &      & mA \\
        $V_\text{inv}$      & Supply voltage\tnote{a}             & 12   &      & 51.0 & V \\
        $V_\text{TVS}$      & TVS\tnote{b}\space{} circuit
                              activation voltage                  & 52   &      & 58   & V \\
        $P_\text{TVS}$      & TVS circuit maximum power
                              dissipation                         &      & 1000 &      & W \\
        $E_\text{TVS}$      & TVS circuit energy absorption
                              capability\tnote{c}                 &      &  5    &      & J \\
        $R_\text{DS-on}$    & FET drain-source on-state resistance &      &  2  &   & $\text{m}\Omega$ \\
                            ...
                            & Motor temperature measurement
                              range\footnote{Depends on thermistor type} & -50  &      & 175  & \degree{}C \\
...
Zubax OpenMyxa is equipped with XT30 battery power connector. 
...
Zubax OpenMyxa measures the temperature of motor windings by means of the KTY84/130 PTC  thermistor. It converts 
and translates measured data via UAVCAN bus\footnote{Not all versions of Telega support motor temperature measurement. 
Refer for Telega documentation for more information}.  
...
Zubax OpenMyxa is equipped with a single RGB LED indicator for purposes of status indication and single Green LED 
for CAN bus transfer indication. These LEDs are located on the PCB near the UAVCAN connectors.

\section{Mechanical characteristics}

The drawings \ref{openmyxa_drawing} and \ref{openmyxa_piccino_drawing} document the basic mechanical characteristics 
of Zubax OpenMyxa,such as dimensions and propeller mounting holes placement.

OpenMyxa is mounting on the chassis like a standard motor. It has standard  19x25 and 25x25 mounting patterns (see Figure \ref{openmyxa_mounting_drawing}).

\begin{figure}[tb]
    \centering    
    \includegraphics[width=0.65\textwidth]{openmyxa_drawing}
    \caption{OpenMyxa drawing.} \label{openmyxa_drawing} 
\end{figure}

\begin{figure}[b]
    \centering    
    \includegraphics[width=0.45\textwidth]{mounting_drawing}
    \caption{OpenMyxa mounting drawing.}\label{Sadulli_mounting_drawing}}
\end{figure}
